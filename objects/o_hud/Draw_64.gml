...
if instance_exists(o_skeleton) {
	draw_hp = lerp(draw_hp, o_skeleton.hp, .2);
	draw_max_hp = o_skeleton.max_hp;
} else {
	draw_hp = lerp(draw_hp, 0, .2);
}
...
draw_line_width_color(hp_x - 1, hp_y - 1.5, hp_x - 1, hp_y + hp_height + .5, 1, c_dkgray, c_dkgray);
draw_line_width_color(hp_x + hp_width, hp_y - 1.5, hp_x + hp_width, hp_y + hp_height + .5, 1, c_dkgray, c_dkgray);
